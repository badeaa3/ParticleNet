...
def main(config = None):
    ...
    # load data
    if ops.background:
        x = get_data({"inFileName" : ops.inFile, "background" : ops.background})
    else:
        x_train, y_train, x_test, y_test = get_data({"inFileName" : ops.inFile, "background" : ops.background})
    ...
    if not ops.background:
        ...
        plt.hist(y_masses[:,:2].flatten(),bins=np.linspace(0,2500,50), histtype="step", color="black", label="truth", density=True)
        plt.hist(y_masses[:,2].flatten(),bins=np.linspace(0,2500,50), alpha=0.5, color="black", label="truth isr", density=True)
    ...
    plt.hist(p_masses[:,:2].flatten(),bins=np.linspace(0,2500,50), histtype="step", color="red", label="prediction", density=True)
    plt.hist(p_masses[:,2].flatten(),bins=np.linspace(0,2500,50), alpha=0.5, color="red", label="prediction isr", density=True)
